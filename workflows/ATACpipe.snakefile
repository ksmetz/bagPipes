...
rule align:
	...
	output:
		...
		sortedBam = temp("output/align/{sampleName}_nodups_sorted.bam"),
	...
	shell:
		...
		java -Xmx16g -jar /nas/longleaf/apps/picard/2.10.3/picard-2.10.3/picard.jar MarkDuplicates I={output.bam} O={output.sortedBam} M={output.dupStats} REMOVE_SEQUENCING_DUPLICATES=true;
		samtools idxstats {output.sortedBam} | cut -f 1 | grep -v 'chrM' | xargs samtools view -b {output.sortedBam} > {output.filteredBam};
